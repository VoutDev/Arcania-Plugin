...
public class GuiBuilder {
    ...
    public GuiBuilder fillEmptySlots(ItemStack fillerItem) {
        for (int i = 0; i < inventory.getSize(); i++) {
            if (inventory.getItem(i) == null || (inventory.getItem(i) != null && Objects.requireNonNull(inventory.getItem(i)).getType().isAir())) {
    ...
    public GuiBuilder addBackButton(Deque<GuiTypeEnum> guiHistory) {
        if (inventory.getHolder() instanceof GuiHolder
            ...
            GuiHelperBase.setPersistentData(PersistentDataEnum.BUTTON_TYPE.toString().toLowerCase(),"back", meta);

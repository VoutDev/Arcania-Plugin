...
public class BlockBreakQueue {
    ...
    private static void startProcessingIfNeeded() {
        ...
        }.runTaskTimer(ArcaniaProvider.getPlugin().getJavaPlugin(), 0L, PROCESS_INTERVAL);
    }
    ...
    private static void processBlockBreak(Player player, BlockBreakData data) {
        ...
        if (data.block.hasMetadata("arcania:veinmined")) {
            data.block.removeMetadata("arcania:veinmined", ArcaniaProvider.getPlugin().getJavaPlugin());
        }
        
        if (data.block.hasMetadata("arcania:quarried")) {
            data.block.removeMetadata("arcania:quarried", ArcaniaProvider.getPlugin().getJavaPlugin());
        }
